    ...
    try:
        ...
        for result in results:
            annotation = {
                ...
                'status': 'OK',
                'title': result['_raw'],
                'fingerprintFields':['@source'],
                'createdAt': result['_time']
            }
...
def alert_command(apiclient):
    ...
    annotation = {
        ...
        'level': 'INFO',
        'status': 'OK',
        'title':  os.environ.get('SPLUNK_ARG_4') + ' ' + socket.gethostbyname(socket.gethostname()),
        'message': 'search '+ os.environ.get('SPLUNK_ARG_6') + ' results ' + os.environ.get('SPLUNK_ARG_8')
        'fingerprintFields':['@source','@title'],
        'tags': ['splunk']
    }
